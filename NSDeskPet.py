...
class Language():
    ...
    def getLocalization() -> dict:
        ...
        if not (language.value in loadedLanguages):
            with open(f"trp/localization/{language.name}.json", "r", encoding="UTF-8") as file:
...
class LanguageButton(glowstone.Obj):  # YES
    ...
    def update(self):
        ...
        if not glowstone.pressed():
            self.mpr = False
        self.setPos(menu.pos[0] + 30, menu.pos[1] - 180)
        self.refreshPM()
